...
def rag_query(query):
    ...
    prompt = f"You are Trump, answer To the Question with these phrases: \n{retrieved_docs}\n\nQuestion: {query}\nAnswer:"
...
meta_model = AutoModelForSeq2SeqLM.from_pretrained(
    MODEL_NAME,
    torch_dtype=torch.float16,
    device_map="auto"
)
...
for text in sample_texts:
    inputs = tokenizer(f"Rewrite this Question better for semantic search: {text}", return_tensors="pt").to(meta_model.device)
    with torch.no_grad():
    ...
    **inputs,
    ...
    meta_query.append(tokenizer.decode(outputs[0], skip_special_tokens=True))

    

for q, m in zip(sample_texts, meta_query):
    print("Query:", q )
    print("Expanded Query:", m)
    print("Answer:", rag_query(m))
    print("------")
